...
echo -e "${BLUE}                                    https://github.com/XH15X ${NC}"
kingtool
...
echo -n -e "H15 >> "
read choice
INSTALL_DIR="/usr/share/doc/XS8TOOL"
BIN_DIR="/usr/bin/"
if [ $choice == 1 ]; then 
	...
	if [[ $? -eq 0 ]]; then
		...
		echo "#!/bin/bash
		python3 $INSTALL_DIR/hackingtool.py" '${1+"$@"}' > XS8TOOL;
		sudo chmod +x XS8TOOL;
		sudo cp XS8TOOL /usr/bin/;
		rm XS8TOOL;
		echo ""; 
    ...
    if [ -d "$INSTALL_DIR" ]; then
        ...
        echo -e $RED   "		[+]+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++[+]"
        echo 		"		[+]						      		[+]"
        echo -e $RED  "		[+]     ✔✔✔ Now Just Type In Terminal (XS8TOOL) ✔✔✔ 	[+]"
        echo 		"		[+]						      		[+]"
        echo -e $RED   "		[+]+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++[+]"
    else
